...
samples = 4  # Set samples to a lower value (fallback to 2 or disable MSAA if needed)
print(f"Maximum supported MSAA samples: {ctx.max_samples}")
if samples > ctx.max_samples:
    print(f"Reducing samples to supported maximum: {ctx.max_samples}")
    samples = ctx.max_samples

# Setup MSAA framebuffer
msaa_fbo = ctx.framebuffer(
    color_attachments=[ctx.texture((WIDTH, HEIGHT), components=4, samples=samples)]
)

fbo = ctx.framebuffer(color_attachments=[ctx.texture((WIDTH, HEIGHT), components=4)])
msaa_fbo.use()
...
output_path = os.path.join(mix_folder, "output_speed.mp4")
...
try:
    while True:
        # Simulate elapsed time based on frame index
        ...
        # Resolve the MSAA framebuffer to the regular framebuffer
        ctx.copy_framebuffer(fbo, msaa_fbo)

        # Read pixels from the resolved framebuffer
