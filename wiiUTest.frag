...
void main() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        float amplitude       = baseAmplitude * mix(1.0, 5.0, random(float(i) + 2.0));
        float frequency       = baseFrequency * mix(0.1, 2.5, random(float(i) + 1.0));

        // Random factor for wave flow
        float waveRandomFactor = mix(0.7, 1.0, random(float(i)));
